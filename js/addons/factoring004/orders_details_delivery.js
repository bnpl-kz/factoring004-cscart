$(function ($) {
  ...
  const container = $('#content_general');
  const showLoader = () => $.toggleStatusBox('show', {overlay: container});
  const hideLoader = () => $.toggleStatusBox('hide');
  const registerHandler = () => container.on('click', '.status-link-c.cm-ajax.cm-post', handleDelivery);
  const unRegisterHandler = () => container.off('click', '.status-link-c.cm-ajax.cm-post', handleDelivery);
